...
export default function BoosterPaper({ booster, moodStyle }) {
  const classes = useStyles();
  const dispatch = useContext(BoosterDispatchContext);

  function handleDelete(id) {
    removeBooster(dispatch, id).catch((e) => console.error(e));
  }

  return (
    <div className={classes.paper} key={booster.id}>
      <Paper className={moodStyle} elevation={10}>
        <Grid
          container
          direction="row"
