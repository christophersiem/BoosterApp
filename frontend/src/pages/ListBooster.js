...
export default function ListBooster() {
  ...
  useEffect(() => {
    fetchBoosterItems(dispatch).catch((e) => console.error(e));
      ...
      <div className={classes.mainPage}>
        ...
        {boosterItems &&
          boosterItems.map((booster) =>
            booster.type === 'JOY' ? (
              <BoosterPaper
                moodStyle={classes.paperJoy}
                booster={booster}
                key={booster.id}
              />
            ) : booster.type === 'CALM' ? (
              <BoosterPaper
                moodStyle={classes.paperCalm}
                booster={booster}
                key={booster.id}
              />
            ) : (
              <BoosterPaper
                moodStyle={classes.paperConf}
                booster={booster}
                key={booster.id}
              />
            )
