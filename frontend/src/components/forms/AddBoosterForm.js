...
import {
  BoosterDispatchContext,
  BoosterStateContext,
} from '../../context/booster/BoosterContext';
...
export default function AddBoosterForm() {
  ...
  function handleSubmit() {
    addBooster(dispatch, boosterToAdd).catch((e) => console.error(e));
  }

  useEffect(() => {
    dispatch({ type: ADD_BOOSTER });
    fetchUserNumbers(userData.userName).then((data) =>
          ...
          {boosterToAdd.owner && (
            ...
            >
              ...
              <Select
                ...
                displayEmpty={true}
              >
                <MenuItem value="JOY" style={{ fontFamily: 'Lora' }}>
                  Joy Booster
                </MenuItem>
                <MenuItem value="CALM" style={{ fontFamily: 'Lora' }}>
                  Calm Booster
                </MenuItem>
                <MenuItem value="CONFIDENCE" style={{ fontFamily: 'Lora' }}>
                  Confidence Booster
                </MenuItem>
              </Select>
            </FormControl>
          ...
          <Grid item>
            <form noValidate autoComplete="off" className={classes.field}>
              <TextField
                ...
                fullWidth={true}
                error={
                  boosterToAdd.name.length > 30 && boosterToAdd.name.length > 0
                }
                helperText={
                  boosterToAdd.name.length > 30 &&
                  boosterToAdd.name.length > 0 &&
                  'Choose a name with maximum 30 characters'
                }
              />
            </form>
          </Grid>
          ...
          {boosterToAdd.name && (
            <Button
              ...
              disabled={
                ...
                !boosterToAdd.owner ||
                !boosterToAdd.type ||
                (!boosterToAdd.youtube &&
                  !boosterToAdd.image &&
                  !boosterToAdd.message)
              }
            >
              Create Booster
            </Button>
